...
"""
function mvar(u::Matrix{<:Real}; maxorder::Union{Nothing, Int} = nothing, criterion::Union{Nothing, String} = "AIC", method::String = "NS", verbose::Bool = true)
...
# ls based estimation of mvar
function mvarls(u::Matrix{<:Real}, order::Int)
    len, nChannels = size(u) 
    # Build up  regressor matrix
    y1 = [zeros(nChannels*order,1); reshape(u',len*nChannels,1)];
    Z =  zeros(nChannels*order,len);
    for i=0:len-1
        Z[:,i+1] = reverse(y1[1+nChannels*i:nChannels*i+nChannels*order]);
    end
    ...
    SU = u' * u - u' * Z' * U1 * u
    SU = SU / (len- nChannels*order-1);
    b = kron(U1, Matrix(I, nChannels, nChannels)) * reshape(u',nChannels*len,1);
    e = reshape(reshape(u,len*nChannels,1) - kron(Z',Matrix(I, nChannels, nChannels)) * b, len, nChannels); 
    
    A = reverse(reshape(b,nChannels,nChannels,order), dims = 2);
    pf = SU #.* len; # see normalization
    ef=e
...
function mvarns(u::Matrix{<:Real}, order::Int)
    ...
    # ISTAT=0;
    # if (order > maxorder)
    #     throw(DomainError("Order $order is larger than max allowed order of 200"))
    # end;
